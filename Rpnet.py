...
class IncResBlock(nn.Module):
    def __init__(self, inplanes, planes, convstr=1, convsize = 15, convpadding = 7):
        ...
        self.conv1_1 = nn.Sequential(
            nn.Conv2d(in_channels = inplanes,out_channels = planes//4,kernel_size = convsize,stride = convstr,padding = convpadding),
            nn.BatchNorm2d(planes//4))
        self.conv1_2 = nn.Sequential(
            nn.Conv2d(inplanes, planes//4, kernel_size=1, stride = convstr, padding=0, bias=False),
            nn.BatchNorm2d(planes//4),
            nn.LeakyReLU(0.2,),
            nn.Conv2d(in_channels = planes//4,out_channels = planes//4,kernel_size = convsize+2,stride = convstr,padding = convpadding+1),
            nn.BatchNorm2d(planes//4))
        self.conv1_3 = nn.Sequential(
            nn.Conv2d(inplanes, planes//4, kernel_size=1, stride = convstr, padding=0, bias=False),
            nn.BatchNorm2d(planes//4),
            nn.LeakyReLU(0.2,),
            nn.Conv2d(in_channels = planes//4,out_channels = planes//4,kernel_size = convsize+4,stride = convstr,padding = convpadding+2),
            nn.BatchNorm2d(planes//4))
        self.conv1_4 = nn.Sequential(
            nn.Conv2d(inplanes, planes//4, kernel_size=1, stride = convstr, padding=0, bias=False),
            nn.BatchNorm2d(planes//4),
            nn.LeakyReLU(0.2,),
            nn.Conv2d(in_channels = planes//4,out_channels = planes//4,kernel_size = convsize+6,stride = convstr,padding = convpadding+3),
            nn.BatchNorm2d(planes//4))
        self.relu = nn.ReLU()
...
class IncUNet (nn.Module):
    def __init__(self, in_shape):
        ...
        self.e1 = nn.Sequential(
            nn.Conv2d(in_channels, 64, kernel_size=3, stride=(1,1),padding=(1,1)),
            nn.BatchNorm2d(64),
            nn.LeakyReLU(0.2,),
            IncResBlock(64,64))
        ...
        self.out_l2 = nn.Sequential(
            #nn.LayerNorm(1920),
            #nn.Linear(in_channels,in_shape[0]*in_shape[1]*in_shape[2]),
            #nn.Linear(in_shape[0]*in_shape[1]*in_shape[2], 2)
            #nn.Flatten(),
            nn.Linear(153600, 2),
            #nn.Sigmoid()
            #nn.Linear(64,2)
    ...
    def forward(self, x):
        ...
        out_1 = self.out_l(de10)
        #import pdb; pdb.set_trace()
        out_1 = out_1.view(-1)
        out = self.out_l2(out_1)
